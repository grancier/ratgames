...
//!
//! Maze generation must be reproducible: the same seed yields the same maze
//! and the same number placement, so a run replays identically and tests are
//! deterministic. This is dependency-free — the domain crate pulls in no
//! `rand` — and uses the same SplitMix64 recurrence as `mathgame-core` and
//! `wordgame-core`, so the sibling domains behave alike without coupling to
//! each other.
